...
def batch_test(model, args):
    ...
    result, hook = calc_accuracy(model=model, loader=test_loader, verbose=True, hter=True)

    hook=hook.cpu().detach().numpy()
    save_csv("surf_vanlia_prediction.txt", hook)
    print(result)


if __name__ == '__main__':
    pretrain_dir = "../output/models/surf_resnet18_se_dropout_no_seed_rgb_version_0.pth"
    args.modal = 'rgb'
    args.gpu = 3
    args.data_root = ""
