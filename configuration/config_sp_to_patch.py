...
parser.add_argument('--method', type=str, default='patch_kd_mmd_avg_sp7_to_ap')

parser.add_argument('--teacher_modal', type=str, default='rgb', help="the origin modal to train the teacher")
...
parser.add_argument('--init_mode', type=str, default='ap',
                    help='the way to init the student net: random, rgb, depth, ir,sp_feature,mse_feature')

parser.add_argument('--lambda_kd', type=float, default=0.5, help='trade-off parameter for kd loss')
...
parser.add_argument('fc_mode', type=str, default='random',help='random,origin,multi')

args = parser.parse_args()
args.name = args.method + "_" + args.teacher_data + "_" + args.student_data + "_lr_" + str(args.lr) + '_version_' + str(
    args.version) + '_weight_patch_' + str(args.weight_patch)
