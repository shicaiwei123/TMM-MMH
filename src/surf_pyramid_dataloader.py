...
surf_pyramid_transforms_train = tt.Compose(
    [
...
#     'RGB')
# img_r = surf_pyramid_transforms_train(img_pil)
# img_b = surf_pyramid_transforms_train(img_pil)
# img_r.show()
# img_b.show()


surf_pyramid_transforms_test = tt.Compose(
    [
        tt.Resize((112, 112)),
        tt.ToTensor(),
        tt.Normalize(mean=[0.5, 0.5, 0.5, ], std=[0.5, 0.5, 0.5, ])
...
def surf_pyramid_dataloader(train, args):
    ...
    if train:
        ...
        surf_dataset = SURF_Multi_Advisor(txt_dir=txt_dir,
                                   root_dir=root_dir,
                                   args=args,transform=surf_pyramid_transforms_train)
        ...
        surf_dataset = SURF_Multi_Advisor(txt_dir=txt_dir,
                                   root_dir=root_dir,
                                   args=args,transform=surf_pyramid_transforms_test)
