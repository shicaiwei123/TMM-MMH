...
class I3D(torch.nn.Module):
    ...
    def forward_single(self, inp):
        ...
        # feat0 = torch.reshape(out, (out.shape[0], out.shape[1] * out.shape[2], out.shape[3], out.shape[4]))
        feat0 = out
        # print(feat0.shape)
        ...
        # feat1 = torch.reshape(out, (out.shape[0], out.shape[1] * out.shape[2], out.shape[3], out.shape[4]))
        feat1 = out
        # print(feat1.shape)
        ...
        # print(out.shape)
        # feat2 = torch.reshape(out, (out.shape[0], out.shape[1] * out.shape[2], out.shape[3], out.shape[4]))
        feat2 = out
        # print(feat2.shape)
        ...
        return out, [feat0, feat1, feat2], out_logits
    ...
    def forward_multi(self, x):
        ...
        for clip_idx in range(x.shape[1]):  # B, 10, 3, 3, 32, 224, 224
            ...
            for crop_idx in range(x.shape[2]):
                ...
                out, feat, out_logits = self.forward_single(clip)
                clip = out_logits
    ...
    def forward(self, batch):
        ...
        if batch[self.index_name].dim() == 5:
            out, feat, out_logits = self.forward_single(batch[self.index_name])
            pred = out_logits
