...
from lib.model_arch_utils import SPP3D
...
class I3D_SPP(torch.nn.Module):
    def __init__(self,
                 num_classes,
                 modality='rgb',
                 dropout_prob=0,
                 name='inception'):
        super(I3D_SPP, self).__init__()

        self.name = name
        self.num_classes = num_classes
        if modality == 'rgb':
            in_channels = 3
            self.index_name = 'frames'
        elif modality == 'flow':
            in_channels = 2
    ...
    def forward_single(self, inp):
        ...
        x_spp = self.spp3d(out)

        feature_num = x_spp.shape[-1]
        patch_score = torch.zeros(x_spp.shape[0], self.num_classes, feature_num)
        ...
        for i in range(feature_num):
            ...
            patch_feature_flatten=torch.unsqueeze(patch_feature_flatten,2)
            patch_feature_flatten=torch.unsqueeze(patch_feature_flatten,3)
            patch_feature_flatten=torch.unsqueeze(patch_feature_flatten,4)
            # print(patch_feature_flatten.shape)
            patch_strength[:, i] = torch.mean(patch_feature_flatten, dim=(1,2,3,4))

            patch_logits = self.conv3d_0c_1x1(patch_feature_flatten)
            patch_logits = patch_logits.squeeze(3)
            patch_logits = patch_logits.squeeze(3)
            patch_logits = patch_logits.mean(2)
        ...
        return out, out_logits, patch_score
    ...
    def forward_multi(self, x):
        ...
        for clip_idx in range(x.shape[1]):  # B, 10, 3, 3, 32, 224, 224
            ...
            for crop_idx in range(x.shape[2]):
                ...
                out, out_logits, patch_score = self.forward_single(clip)
                clip = out_logits
    ...
    def forward(self, batch):
        ...
        # print(batch[self.index_name].dim())
        if batch[self.index_name].dim() == 5:
            out, out_logits, patch_score = self.forward_single(batch[self.index_name])
            pred = out_logits

        # 7D tensor == 3 crops/10 clips
        elif batch[self.index_name].dim() == 7:
            pred = self.forward_multi(batch[self.index_name])
            patch_score=pred
        ...
        return pred, patch_score, loss_dict
