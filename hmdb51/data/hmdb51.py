...
class HMDB51(Dataset):
    def __init__(self, data_root, split_path, clip_len, mode, sample_interal=2):
        ...
        if self.train:
            self.clip_transform = util.clip_transform_hmdb('train', clip_len)
        else:
            self.clip_transform = util.clip_transform_hmdb('val', clip_len)
    ...
    def sample_all(self, entry):
        ...
        if len(flow_x) > self.clip_len:
            ...
            if self.train:  # random sample
                ...
                # offset = np.random.randint(self.clip_len // 2, len(flow_x) - self.clip_len - self.clip_len // 2)
                # rgb_img = rgb_img[offset:offset + self.clip_len]
                # flow_x = flow_x[offset - self.clip_len // 2:offset + self.clip_len + self.clip_len // 2]
                # flow_y = flow_y[offset - self.clip_len // 2:offset + self.clip_len + self.clip_len // 2]
            else:  # center crop
                offset = len(flow_x) // 2 - self.clip_len // 2
                rgb_img = rgb_img[offset:offset + self.clip_len]
                flow_x = flow_x[offset:offset + self.clip_len]
...
class HMDB51_Multcrop(HMDB51):
    def __init__(self, data_root, split_path, clip_len, mode, sample_interal=2):
        ...
        :param sample_interal: 数据抽样加快训练
        '''
        super(HMDB51_Multcrop, self).__init__(data_root=data_root, split_path=split_path, clip_len=clip_len, mode=mode)

        self.clip_transform = util.clip_transform_hmdb('3crop', self.clip_len)
    ...
    def __getitem__(self, index):
        if self.mode == 'rgb':
            ...
            for clip in clips:
                ...
                clip = torch.stack(clip, 0)  # (3, 3,16, 224, 224)
                frames.append(clip)
            frames = torch.stack(frames, 0)  # (10, 3, 3,16, 224, 224)
