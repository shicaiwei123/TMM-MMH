...
def save(net, optimizer, epoch, iteration, model_dir, args):
    ...
    path_cell = args.rgb_pretrain_path.split('/')
    save_dir = os.path.join("../results", args.method, path_cell[2], path_cell[3], 'all', args.init_mode,
                            'lambda_kd_' + str(args.lambda_kd), 'weight_patch_' + str(args.weight_patch))
...
def train_knowledge_distill_action(net_dict, cost_dict, optimizer, train_loader, test_loader, args):
    ...
    while iteration <= args.max_iter:
        ...
        for batch in train_loader:
            ...
            depth_batch = {"flow": batch["flow"], 'label': batch["label"]}
            ...
            if args.weight_patch:
                kd_loss = criterionKD(pred_spp_student, pred_spp_teacher.detach(), weight=pred_spp_student)
            else:
            ...
            if iteration % args.print_every == 0:
                ...
                print(log_str + '|%f' % clc_loss, '|%f' % kd_loss)
                # print()
        ...
        epoch += 1
        loss_meters['bAcc'].reset()
        loss_meters['total_loss'].reset()
        ...
        with torch.no_grad():
            if epoch >= 20:
                ...
                if score > score_best:
                    score_best = score
                    save(student_model, optimizer=optimizer, epoch=epoch, iteration=2, model_dir=args.model_root,
                         args=args)
            student_model.train()
        if epoch > 250:
            sys.exit(1)
...
def train_knowledge_distill_action_ensemble(model, optimizer, test_loader, train_loader, args):
    '''
    ...
    while iteration <= args.max_iter:
        ...
        with torch.no_grad():
            score = test(model=model, test_dataloader=test_loader)
        print(score)
        if score > score_best:
            score_best = score
            save(model, optimizer=optimizer, epoch=epoch, iteration=2, model_dir=args.model_root,
                 args=args)
...
def train_knowledge_distill_action_single(net_dict, cost_dict, optimizer, train_loader, test_loader, args):
    '''
    ...
    criterionCls = cost_dict['criterionCls']
    criterionKD = cost_dict['criterionKD']
    from loss.kd.dask import DASK

    if args.method == 'mars' or args.method == 'mers':
        kl_loss_func = nn.MSELoss()
    if args.method == 'adda':
        kl_loss_func = PKT()
    if args.method == 'dhad':
        kl_loss_func = criterionKD
    if args.method == 'dask':
        kl_loss_func = DASK()
    ...
    if torch.cuda.is_available():
        ...
        bce_loss = nn.BCELoss().cuda()
        mse_loss = nn.MSELoss().cuda()
        kl_loss_func = kl_loss_func.cuda()
    ...
    while iteration <= args.max_iter:
        ...
        for batch in train_loader:
            ...
            if args.teacher_data == 'flow':
                teacher_batch = {"flow": batch["flow"], 'label': batch["label"]}
                student_batch = {"frames": batch["frames"], 'label': batch["label"]}
            else:
                teacher_batch = {"frames": batch["frames"], 'label': batch["label"]}
                student_batch = {"flow": batch["flow"], 'label': batch["label"]}

            if torch.cuda.is_available():
                teacher_batch = util.batch_cuda(teacher_batch)
                student_batch = util.batch_cuda(student_batch)
            with torch.no_grad():
                pred_teacher, feat_teacher, teacher_loss_dict = teacher_model(teacher_batch)
            pred_student, feat_student, student_loss_dict = student_model(student_batch)

            loss_dict = {k: v.mean() for k, v in student_loss_dict.items()}
            clc_loss = sum(loss_dict.values())

            if args.method == 'dhad':
                kd_loss = criterionKD(feat_student[0], feat_teacher[0], feat_student[1], feat_teacher[1],
                                      feat_student[2], feat_teacher[2], batch['label'].cuda())
            else:
                feat_student = feat_student[2]
                feat_teacher = feat_teacher[2]

                if args.method in ['adda','dask']:
                    feat_student = feat_student.view(feat_student.shape[0], -1)
                    feat_teacher = feat_teacher.view(feat_teacher.shape[0], -1)

                kd_loss = kl_loss_func(feat_student, feat_teacher.detach())

            if args.method == 'mers':
                loss = kd_loss * args.lambda_kd
            else:
                loss = clc_loss + kd_loss * args.lambda_kd
            ...
            if iteration % args.print_every == 0:
                log_str = 'iter: %d (%d + %d/%d) | ' % (iteration, epoch, iteration % total_iters, total_iters)
                log_str += ' | '.join(['%s: %.3f' % (k, v.value()[0]) for k, v in loss_meters.items()])
                print(log_str, '|%f' % clc_loss.item(), '|%f' % kd_loss.item())
