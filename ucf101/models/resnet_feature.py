...
class I3Res50_feature(nn.Module):
    ...
    def forward(self, batch):
        ...
        if batch['frames'].dim() == 5:
            pred, feature = self.forward_single(batch['frames'])
        ...
        return pred, feature, loss_dict


# -----------------------------------------------------------------------------------------------#

def i3_res50_feature(num_classes, pretrain_dir):
    net = I3Res50_feature(num_classes=num_classes, use_nl=False)

    state_dict = torch.load(pretrain_dir)
    if num_classes != 400:  # 自己训练的模型
...
def i3_res50_nl(num_classes):
    net = I3Res50_feature(num_classes=num_classes, use_nl=True)
