...
class ResNet(nn.Module):
    ...
    def forward(self, x, cluster_center):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        # pacth
        feature_list = self.roi_pooling(x, cluster_center, spatial_ratio=28)
...
def resnet18_se_sift_patch(args, pretrained=False, **kwargs):
