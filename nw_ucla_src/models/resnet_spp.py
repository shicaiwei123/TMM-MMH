...
class I3Res50_SPP(nn.Module):
    ...
    def forward_single(self, x):
        ...
        patch_score = torch.zeros(x_spp.shape[0], feature_num, self.class_num)
    ...
    def forward(self, batch):
        ...
        # print(batch['frames'].dim())

        # 5D tensor == single clip
        if batch['frames'].dim() == 5:
            pred, pred_spp = self.forward_single(batch['frames'])
        ...
        return pred, pred_spp, loss_dict


# -----------------------------------------------------------------------------------------------#

def i3_res50_spp(num_classes, pretrain_dir):
    net = I3Res50_SPP(num_classes=num_classes, use_nl=False)

    if len(pretrain_dir) == 0:
        return net
    else:
