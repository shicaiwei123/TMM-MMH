...
def train_knowledge_distill_action(net_dict, cost_dict, optimizer, train_loader, test_loader, args):
    ...
    while iteration <= args.max_iter:
        for batch in train_loader:
            ...
            # rgb_batch_val = {"frames": batch["frames_rgb_val"], 'label': batch["label"]}
            # depth_batch_val = {"frames": batch["frames_depth_val"], 'label': batch["label"]}
            # rgb_batch_train = {"frames": batch["frames_rgb_train"], 'label': batch["label"]}
            #
            # if torch.cuda.is_available():
            #     rgb_batch_val = util.batch_cuda(rgb_batch_val)
            #     depth_batch_val = util.batch_cuda(depth_batch_val)
            #     rgb_batch_train = util.batch_cuda(rgb_batch_train)
            #
            # pred_teacher, pred_spp_teacher, _, _ = teacher_model(rgb_batch_val, depth_batch_val)
            # pred_student, pred_spp_student, student_loss_dict = student_model(rgb_batch_train)
        ...
        # model_dir = os.path.join(args.model_root, str(args.split_num))
        # makedir(model_dir)
        #
        # print(acc_average)
...
def train_knowledge_distill_action_feature(net_dict, criterions, optimizer, train_loader, test_loader, args):
    ...
    while iteration <= args.max_iter:
        for batch in train_loader:
            ...
            pred_teacher, teacher_feature, _, _ = teacher_model(rgb_batch_val, depth_batch_val)
            pred_student, student_feature, student_loss_dict = student_model(rgb_batch_train)
    ...
    while iteration <= args.max_iter:
        ...
        if epoch > 0:
            ...
            with torch.no_grad():
                ...
                if score > score_best:
                    score_best = score
                    save(model, optimizer=optimizer, epoch=epoch, iteration=2, model_dir=args.model_root, args=args)
