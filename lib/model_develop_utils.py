...
def calc_accuracy(model, loader, verbose=False, hter=False):
    ...
    hook = torch.cat((outputs_full, labels_full.unsqueeze(1)),dim=1)
    ...
    if hter:
        ...
        try:
            ...
            return [accuracy, 1, 1, 1, 1, 1], hook

        return [accuracy, FAR, FRR, HTER, APCER, NPCER], hook
    else:
        return [accuracy], hook
...
def calc_accuracy_ensemble(rgb_model, depth_model, dataloader, verbose=False, hter=False):
    ...
    outputs_full = []
    labels_full = []
    for (batch_sample, single_data, label) in tqdm(iter(dataloader), desc="Full forward pass", total=len(dataloader),
                                                   disable=not verbose):
...
def train_base(model, cost, optimizer, train_loader, test_loader, args):
    ...
    if args.lr_decrease == 'cos':
        ...
        cos_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[np.int(1/6*args.train_epoch),
                                                                              np.int(2/6*args.train_epoch),
                                                                              np.int(3/6*args.train_epoch)])
        if args.lr_warmup:
            scheduler_warmup = GradualWarmupScheduler(args, optimizer, multiplier=1,
                                                      after_scheduler=cos_scheduler)
