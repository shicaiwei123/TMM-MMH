...
class SPP3D(nn.Module):
    def __init__(self):
        ...
        self.normal_pooling = nn.AdaptiveMaxPool3d((2, 4, 4))
        self.pooling_2x2 = nn.AdaptiveMaxPool3d((1, 2, 2))
        self.pooling_1x1 = nn.AdaptiveMaxPool3d((1, 1, 1))
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, dim_in, dim_k, dim_v):
        ...
        self.dim_k = dim_k
        self.dim_v = dim_v
        self.linear_q = nn.Sequential(nn.Linear(dim_in, dim_k, bias=False))
        self.linear_k = nn.Sequential(nn.Linear(dim_in, dim_k, bias=False))
        self.linear_v = nn.Sequential(nn.Linear(dim_in, dim_k, bias=False))
    ...
    def forward(self, q, x):
        ...
        # save_csv("dist_attention2.csv", dist_save.cpu().detach().numpy())

        dim_in = x.shape
        # print(self.dim_in,dim_in)
